...
{
    sc1(v2[i]);
    if(pos.find(v2[i]) == pos.end()) pos[v2[i]] = 0, ck = 1;
}
...
ll mx, mn, range;
    ...
    if(range > 0)
    {
        if(v[r] > mx) cost += (range / k) * x + (range % k) * y;
            ...
            if(range < k) ck = 1;
            else cost += ((range / k) - 1) * x + (range % k) * y + 1 * xc;
            }
    }
...
for(int i = 2; i <= m; i++)
{
    ...
    if(range > 0)
    {
        if(v[l] > mx) cost += (range / k) * x + (range % k) * y;
            ...
            if(range < k) ck = 1;
            else cost += ((range / k) - 1) * x + (range % k) * y + 1ll * xc;
        }
    }
...
///    *check for n=1
// phi(n * m * x) = phi(n) * phi(m) * phi(x)

// n = p * p * q = a * q = phi(a * q) = phi(a) * phi(q)
// phi(a) = a * (p - 1) / p,
// phi(q) = q * (q - 1) / q.
