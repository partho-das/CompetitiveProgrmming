...
int gauss(int n, int m, double ar[MAXROW][MAXCOL], vector<double>& res){ /// hash = 835176
    ...
    for (j = 0, i = 0; j < m && i < n; j++){ // j working on columns
        for (k = i, p = i; k < n; k++){      // finding the row with max value
            if (abs(ar[k][j]) > abs(ar[p][j])) p = k;
        }

        if (abs(ar[p][j]) > EPS){
            pos[j] = i;
            for (l = j; l <= m; l++) swap(ar[p][l], ar[i][l]); // swapping with row with max val

            for (k = 0; k < n; k++){
                if (k != i){
                    double x = ar[k][j] / ar[i][j]; // reducing other rows
    ...
    for (i = 0; i < m; i++){
        if (pos[i] == -1) free_var++;
        else res[i] = ar[pos[i]][m] / ar[pos[i]][i]; // dividing by coefficient
    }
